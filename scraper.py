...
def scrape(language, filename):
    ...
    with codecs.open(filename, "a", "utf-8") as f:
	    ...
	    star = i("a.muted-link:first").text()
            url = i("h3 a").attr("href")
            url = "https://github.com" + url
            # ownerImg = i("p.repo-list-meta a img").attr("src"):star:
            # print(ownerImg)
            f.write(u"* [{title}]({url}): {description} :star:{star}\n".format(title=title, url=url, description=description, star=star))
